...
test("real dynamic release surfaces keep runtime renderers and fail-closed demo actions", () => {
  ...
  const commercialPresentation = readFileSync("lib/commercial/commercial-presentation.ts", "utf8");
  ...
  assert.match(commercialPresentation, /offer\.dataClassification === "PUBLISHED_RECORD"/);
  assert.match(bestOffers, /card\.action \?/);
  assert.match(bestOffers, /messages\.common\.reviewOnly/);
  assert.match(visualFixture, /process\.env\.VERCEL !== "1"/);
  assert.match(visualFixture, /process\.env\.VERCEL_ENV !== "production"/);
