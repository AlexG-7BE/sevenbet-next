...
| --- | --- | --- | --- | --- | --- |
| Prisma Postgres | Detected | Redacted hosted endpoint classification plus Prisma datasource/repositories. **Detected:** both isolated resources are on Free and expose no managed backup. A Preview logical backup restored successfully to a disposable local PostgreSQL 16 target; this does not prove provider-native or Production recovery. | High | `DATABASE_URL`, `DIRECT_URL`, provider aliases | Founder Office/config owner; technical consumer repository maintainer |
