...
5. **State the Editor Score method once** in
   `lib/casino-global-catalog/editor-score.ts`, and use it to fill a missing
   score rather than to replace an existing one.

## Why point 5 is not a blanket recompute

The Founder first asked, on 23 September, for every score to be recomputed over
the completed record. Running the recompute in preview showed what that would
actually do, and the Founder then decided against it the same day.
...
languages. It does not measure the quality of a casino. It reproduces all
thirteen EGO scores exactly, because those were generated by it. The other
fourteen were Founder editorial judgements from
CASINO-REAL-CATALOG-02/03, and recomputing them replaces a human call with a
count of how much evidence we happen to hold:

| Casino | Published | Recomputed |
| --- | --- | --- |
| Inkabet | 9.0 | 7.6 |
| SuperCasino | 8.6 | 7.2 |
| Betsson | 8.8 | 7.5 |
...
| Betsafe | 8.8 | 7.9 |
| Skol Casino | 8.4 | 7.5 |
| All 13 EGO casinos | — | unchanged |

Inkabet is a strong single-market Peruvian brand; one market and four
categories score 7.6 however good it is. Applying this across the catalogue
would compress the published range from 7.2–9.0 to 7.0–8.2, erase the
differentiation those fourteen judgements carry, and feed that flattening into
the new ranking. The executor therefore fills a missing score and refuses to
overwrite one unless a caller names a casino explicitly.

Explainability is delivered instead through the rewritten "Why we rate it"
section on each profile and the new ranking reason on each directory card,
which state the same signals in the reader's terms. A public per-component
breakdown was considered and deferred: the same six numbers would mean "how the
score was calculated" on thirteen casinos and "what our record covers" on the
other fourteen, which is worse than not showing them.
...
Counts run over the casino's AVAILABLE markets together with its global catalog
layer. A market recorded as unavailable describes somewhere the casino is not
offered and must not lift the score, and a licence no available market
evidences cannot raise trust.
...
`plan`, `apply`, `editorial`, `score-plan` and `scores` commands. See
