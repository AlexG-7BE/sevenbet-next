...
Release is an ordered Founder-controlled operation. Before merge, Founder/Operations sets and verifies the three Production-only values without triggering a deployment or otherwise mutating the current Production application. Founder then merges by merge commit; the automatic exact-main Vercel Production deployment must build with that environment contract. After the exact merged SHA is Ready, verify apex, `www` redirect, canonical, Organization schema, OpenGraph, robots, sitemap, Better Auth session behaviour, safe account smoke, legacy-host behaviour and runtime errors. Only then may BRAND-CUTOVER-01 be closed. Google OAuth activation remains off until after this verification, and Preview remains unchanged.
