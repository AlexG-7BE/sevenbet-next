...
| Resend Contact delivery | Narrow transactional delivery from the public Contact endpoint to the existing support mailbox; separate from account/Programme communications | Visitor name if supplied, email, subject, message, transactional sender/recipient and delivery metadata; no Programme, Self-Check, Protected Help, casino-personalisation or marketing data | **Detected in code; external activation not detected:** direct server-side HTTPS adapter, exact enable flag, approved From/To allow-list, plain text only, no automatic retry, tracking or application-database persistence | Provider role, exact contracting entity, DPA, locations, subprocessors, support access, retention, deletion, security evidence and UK transfer mechanism **not verified**. DNS/domain and real delivery remain deferred/blocked until RFC-028 gates pass. |
| Account and Programme email delivery | Founder-approved Resend processing for transactional/account, welcome, consented Programme reminder and consented marketing purposes; Production delivery enabled after controlled acceptance | Minimum recipient, approved rendered message, sender/reply-to and opaque correlation data only; no Programme answers, vulnerability data, analytics history, affiliate tokens, browsing history, raw IP or auth/password token | **Detected:** RFC-046 Production ledgers and adapter, PRs #271/#273/#274, live domain/DKIM/SPF, sending-only credentials, exact five-event signed webhook, Production sender configuration, five provider-delivered fixture messages and six of six controlled acceptance cases | Founder authority and activation evidence were recorded 2026-09-12. Exact agreement acceptance/entity, location, retention, subprocessors and UK transfer mechanism remain **open**. |
...
| OpenAI API | Preview-only live/fallback voice transcription and bounded Programme text generation under RFC-023/RFC-056 | Live microphone audio and partial/final transcript for preferred voice; current in-memory completed audio file for fallback; bounded Programme situation/structural facts and generated result; technical token/tier/latency counts | **DETECTED in candidate code; successful live call not detected:** server adapters use `gpt-live-transcribe`, fallback `gpt-4o-transcribe` and `gpt-6-luna`; Responses use `store=false`, no background/conversation/previous response/tools; realtime uses server-mediated SDP and no client credential; no application database retention of raw audio/transcript/provider messages | Exact contracting entity, DPA, controller/processor roles, account data-sharing/training setting, locations, subprocessors, support access, deletion and UK transfer mechanism **UNKNOWN**. Official current defaults permit up to 30-day abuse-monitoring retention for Responses/Realtime; account controls were not inspected. Founder/team/synthetic Preview only; unrestricted Production data remains blocked pending the governed retention position. |

## Not detected

- No advertising SDK, affiliate pixel/postback processor, APM provider,
  external queue or automated paging provider is detected in the active
  application dependencies. First-party consented analytics is active and
  Resend lifecycle processing is active after bounded acceptance. Google
  identity code does not add a Google client SDK or mailbox
  API. The OpenAI integration uses platform fetch and adds no provider SDK
  dependency.
