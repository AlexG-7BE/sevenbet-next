...
| GB jurisdiction and operator evidence authority | Implemented, non-commercial policy | `lib/jurisdiction/`, public casino/offer/comparison services, `/r/[slug]`, `/go/[slug]` | Repository-controlled versioned GB policy plus existing Casino/Affiliate records | Current policy denies commercial/referral; licensed-domain evidence, COMM-01 and LEGAL-02 are not complete. |
...
- **Implementation completeness estimate: 58%** (historical engineering estimate, not a measurement). The application now also has a bounded technical GB authority and proven Preview/Production isolation. Completeness remains reduced by real licensed-domain/partner evidence, external legal approval, provider recovery evidence, Programme operational gaps and unconfirmed Production integrations.
