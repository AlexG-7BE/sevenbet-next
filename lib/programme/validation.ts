...
export function parseActiveBoundary(
  ...
  if (complete) {
    ...
    if (scenarioAnswer !== correctBoundaryScenarioAnswer) {
      throw new ValidationError("The boundary scenario check must be answered correctly");
    }
    const missing = boundaryStrengthChecks.filter((item) => !checks?.includes(item));
    if (missing.length) {
      throw new ValidationError("Every boundary strength check is required", { fields: missing });
    }
  }
