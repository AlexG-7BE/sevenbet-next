...
| 24 | RECOVERY-01 | **PARTIAL — MANAGED CANARY SNAPSHOT PENDING** | RFC-024 approved. Starter and seven-day retention metadata apply to both isolated databases; 14 completed Production snapshots and six completed Preview snapshots are detected. A completed Preview snapshot restored provider-natively into a new disconnected Prisma database, passed exact 18-migration/schema/FK/structure/repository-read validation and was deleted by exact ID. The selected snapshot predates the one pending synthetic Preview canary, so canary parity awaits a later snapshot. |
