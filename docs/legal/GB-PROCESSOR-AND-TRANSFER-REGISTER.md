...
- Resend delivers Contact messages to the documented Google Workspace support
  ...
  normalized webhook and message store. The 12 September activation candidate
  adds a protected-cron worker, while Production delivery remains disabled.
...
| RFC-046 first-party analytics | Existing Vercel hosting and Prisma Postgres receive bounded events only after consent; not a separate analytics provider | ACTIVE / PRODUCTION VERIFIED |
