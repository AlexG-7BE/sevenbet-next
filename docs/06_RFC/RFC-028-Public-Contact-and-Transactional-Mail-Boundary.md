...
The first 2026-08-13 Preview attempt consumed its one-send allowance after the browser normalized a CR/LF test subject. Provider acceptance and delivery passed, and the then-approved From/To were exact, but Reply-To was `visitor@example.invalid` rather than a Founder-controlled address. Delivery was rolled back off. Founder Office correction dated 2026-08-13 preserves that history and separately authorised exactly one replacement Preview submission only after the sender, Workspace alias, authentication, DMARC, disabled deployment and non-send gates passed.

The corrected replacement was sent exactly once on 2026-08-13 after Google Admin showed `info@b4gamble.com` as an alternate address on the existing `support@b4gamble.com` user and the corrected Preview reached Ready. Vercel and Resend recorded one successful provider request and a delivered message with exact From `B4GAMBLE <info@b4gamble.com>`, To `support@b4gamble.com` and a Founder-controlled external Preview test address as Reply-To; no CC/BCC or visitor confirmation was present. Founder Office subsequently confirmed actual receipt in `support@b4gamble.com`, satisfying `FOUNDER_MAILBOX_CONFIRMATION_REQUIRED`, and authorised WAF, fail-closed Production preparation, merge and the bounded Production verification sequence. No further Preview message is permitted.

The Production contract is exact From `B4GAMBLE <info@b4gamble.com>`, To `support@b4gamble.com`, and Reply-To equal to the validated email entered by the real visitor. The single authorised Production smoke must instead use the B4GAMBLE-controlled synthetic visitor identity `B4GAMBLE Production Test` with `info@b4gamble.com` as its submitted email/Reply-To, subject `B4GAMBLE Contact Production verification`, and message `B4GAMBLE-controlled non-sensitive Production verification. No customer data.` No personal external address may enter Production configuration, defaults, tests, runtime configuration or future smoke submissions.
