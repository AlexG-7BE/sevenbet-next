...
| --- | --- | --- | --- | --- | --- |
| Prisma Postgres | Detected | Redacted hosted endpoint classification plus Prisma datasource/repositories. **Detected:** the Vercel-billed workspace and both isolated resources are Starter; seven-day retention metadata and completed Production/Preview snapshots are present. A Preview snapshot restored provider-natively into a new isolated database, passed exact validation and was deleted by immutable ID. RECOVERY-01 remains partial only because the selected point predates the pending synthetic canary. | High | `DATABASE_URL`, `DIRECT_URL`, provider aliases | Founder Office/config owner; technical consumer repository maintainer |
