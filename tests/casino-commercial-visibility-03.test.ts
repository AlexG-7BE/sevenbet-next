...
test("15. complete detail data uses global evidence instead of false Not listed states", () => {
  ...
  assert.match(profile, /commercialLabels\.noCurrentOffer/);
  assert.match(profile, /casino\.offerPresentation\?\.relation === "EXACT" \|\| casino\.offerPresentation\?\.relation === "ROW"/);
  assert.match(profile, /bonus\.importantConditions\.slice\(0, 2\)\.map/);
});
