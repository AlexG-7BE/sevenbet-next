...
2. OAuth uses the existing Better Auth staff identity with authorization code, PKCE S256, explicit consent, short-lived opaque access tokens, rotating refresh tokens, revocation, exact client/resource binding and current ChatGPT-compatible public-client registration. A valid consumer identity is insufficient; `AdminUser` and `affiliate.manage` are checked at authorization, token/refresh and resource use.
3. The existing consumer, Google identity-only, Admin session and Programme authentication semantics remain unchanged. The provider tables are one additive migration. The MCP feature is explicitly gated and fails closed without affecting ordinary application startup or CRM use.
