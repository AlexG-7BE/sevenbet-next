...
const sections = [
  ...
  },
  {
    ...
    content: <><p>We use service providers only for defined operational purposes: Vercel for hosting and application infrastructure; Prisma&apos;s managed Postgres service for application data; Google for identity-only sign-in; Resend for Contact email delivery; Google Workspace for the support mailbox; and OpenAI for optional AI and transcription after your explicit consent. Authorised B4GAMBLE personnel may access information where needed to operate, secure or support the service.</p><p>Where a published offer uses an original vetted partner-hosted creative, your browser requests that media from Superfly or Bannerflow. The provider receives ordinary request information such as IP address, user agent and the creative identifiers needed to render or measure the media. B4GAMBLE sends these requests without a referrer and does not send Programme, protected Help, account or commercial-eligibility data as creative parameters.</p><p>A Contact message is delivered to the B4GAMBLE support mailbox through Resend. We do not create a marketing permission from your message. We do not sell personal data.</p><p>We may disclose information if law requires it, to protect legal rights or safety, or in connection with a corporate transaction subject to appropriate protections.</p></>,
  },
  {
    id: "transfers",
  ...
  },
  {
    ...
    content: <><p>This service uses only cookies or browser storage needed for a service you request, authentication, security, access confirmation, language preference, same-tab comparison selection and temporary Programme continuity. The presentation cookie stores language only; it does not store your country, raw IP address or commercial eligibility. We do not run non-essential product analytics or session replay.</p><p>A vetted partner-hosted creative can make a no-referrer media or impression request to Superfly or Bannerflow. In the exact provider fixtures evaluated before this update, no response set a cookie and no cookie or browser-storage write was observed; the isolated Bannerflow frame cannot read B4GAMBLE&apos;s page or storage. There is therefore no optional storage choice to present for the currently observed behavior. We revalidate new provider behavior and will apply the existing choice boundary before loading if a future creative uses non-essential storage or access that requires it.</p><p>Necessary B4GAMBLE storage is not reused for advertising, affiliate measurement or commercial personalisation. Browser controls can remove stored data, but disabling necessary storage may prevent requested language, account or Programme features from working.</p></>,
  },
  {
    id: "affiliate",
