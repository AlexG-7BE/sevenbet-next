...
export class AffiliateRedirectService {
  ...
  async resolve(slugValue: string, input: AffiliateRedirectRequestInput = {}): Promise<AffiliateRedirectResolution> {
    ...
    const trustedSignalMatches = input.requestCountrySignal?.trust === "TRUSTED"
      && input.requestCountrySignal.countryCode?.trim().toUpperCase() === countryCode;
    const marketCode = canonicalCommercialMarketKey({
      countryCode,
      marketCode: trustedSignalMatches
        ? input.requestCountrySignal?.marketCode ?? countryCode
        : null,
      trust: trustedSignalMatches ? "TRUSTED" : "UNTRUSTED",
    });
    const activation = marketCode
    ...
    const verifiedCanonicalDestination = () => validateRedirectTargetUrl(routing.destination.href);

    if (jurisdictionDecision.countryCode !== "GB") {
      const destination = verifiedCanonicalDestination();
      if (!destination) return { ok: false, reason: "UNSAFE_REDIRECT_URL", slugId: routing.slugId, casinoId: routing.casinoId, candidates: routing.candidates, jurisdictionDecision };
    ...
    if (!operatorEligibility.operatorEvidenceEligible) {
      return { ok: false, reason: "OPERATOR_EVIDENCE_DENIED", slugId: routing.slugId, casinoId: routing.casinoId, candidates: routing.candidates, jurisdictionDecision, operatorEligibility, commercialReadiness };
    }
    ...
    const destination = verifiedCanonicalDestination();
    if (!destination) return { ok: false, reason: "UNSAFE_REDIRECT_URL", slugId: routing.slugId, casinoId: routing.casinoId, candidates: routing.candidates, jurisdictionDecision, operatorEligibility, commercialReadiness };
