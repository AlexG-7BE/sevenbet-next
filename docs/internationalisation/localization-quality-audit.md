...
| `/outbound/[slug]` | Server compatibility redirect | no; internal action boundary | `GB_ONLY` | Managed-slug syntax only; `/r/[slug]` owns all commercial authority | No independent commercial authority | No rendered UI | Invalid slug redirects to `/outbound/unavailable`; a valid slug redirects internally to `/r/[slug]` | `/r/[slug]` retains fail-closed recovery | `noindex, nofollow`; normal public UI does not link here |
