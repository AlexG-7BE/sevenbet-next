...
| --- | --- | --- | --- |
| Provider recovery capability | **Detected:** Starter, seven-day retention metadata, completed Production/Preview snapshots and a validated provider-native Preview restore to a new exact-ID-deleted target. The selected snapshot predates the pending deterministic canary, so managed canary parity is not yet proven. | Wait for a later completed Preview snapshot, restore it to a fresh isolated target, prove exact canary parity, then delete the canary and target by immutable ID. | High |
