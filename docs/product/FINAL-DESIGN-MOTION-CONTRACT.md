...
RFC-035 supersedes the original Home wheel-tween and Home rise-timing rows only. Its 2026-08-19 Founder refinement preserves browser ownership while adding native proximity fixation at deliberate major compositions; it does not restore input interception.
...
| Home | Major narrative compositions | Native wheel, trackpad, keyboard or touch scroll ends near an approved target | User-controlled physical scroll position | Browser may settle to the nearby Hero, Recognition, plan, chapter, evidence, trust or final-composition boundary | Browser-native `y proximity`; `scroll-snap-stop: normal` | Browser-native | Fine and coarse pointers; stronger input may skip targets | Snap disabled; native direct scroll | Home handoff CSS; no JS scroll controller |
...
- Home proximity fixation is CSS-only, targets major compositions rather than prototype percentage markers, uses normal stop behaviour and never cancels or delays wheel, touch or keyboard input.
