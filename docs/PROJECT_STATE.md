...
- **GOOGLE-OAUTH-ACTIVATE-01:** RFC-021 v2.1 correction candidate on `codex/google-oauth-activate-01-b4gamble`; consolidates Programme access confirmation, adds current server-verifiable bounded auth authority, preserves separate OAuth/content-claim continuation, makes actual Better Auth session state authoritative for Programme home/header routing, and canonicalises the exact Vercel Preview deployment host to the exact stable branch host before application or Better Auth handling. External Preview credential reconfiguration and real-provider E2E remain Founder-controlled.
