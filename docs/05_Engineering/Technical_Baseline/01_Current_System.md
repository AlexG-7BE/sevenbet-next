...
| Authentication and staff administration | Implemented, Google externally inactive | Better Auth handler, identity-only Google account hooks, restricted auth paths, public Programme sign-out, staff/profile checks, bootstrap scripts | Prisma User/Session/Account/AdminUser; Google rows retain identity association without durable OAuth token/scope material | Legacy preview-token fallback remains available only when explicitly enabled; real Google E2E awaits separate external activation. |
