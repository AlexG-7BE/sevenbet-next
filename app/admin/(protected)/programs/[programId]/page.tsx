...
import { ProgramWorkflowActions } from "@/components/admin/ProgramWorkflowActions";
...
export const metadata: Metadata = { title: "Program Dashboard | B4GAMBLE CMS", robots: { index: false, follow: false } };
...
export default async function ProgramDashboard({ params }: { params: Promise<{ programId: string }> }) {
  ...
  return (
    <AdminPageShell title={snapshot.program.title} intro="Program health, workflow, versions and recent structural changes." actions={<div className="heroActions">{canAccessAdminArea(staff, "program-preview") ? <Link className="button ghost" href={`/admin/programs/${programId}/preview`}>Preview</Link> : null}{canAccessAdminArea(staff, "program-edit") ? <Link className="button gold" href={`/admin/programs/${programId}/builder`}>Open builder</Link> : null}</div>}>
      ...
      <div className="adminPanelGrid">
        ...
        <Card><Badge>{snapshot.achievements.length} achievements</Badge><h2>Learning rewards</h2><p className="muted">XP is awarded by idempotent event keys. Rule edits do not rewrite previously earned user XP.</p>{canAccessAdminArea(staff, "xp-rules") ? <Link href="/admin/xp-rules">Manage XP rules</Link> : null}</Card>
