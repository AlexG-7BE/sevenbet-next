...
- **Status:** Approved
- **Decision authority:** Founder Office `GOOGLE-OAUTH-ACTIVATE-01` Preview flow correction v2.1, Preview canonical-host correction, client authority clock-skew correction, post-OAuth claim-redemption correction and final subject-transition correction
- **Approved:** 2026-08-10
...
After authentication succeeds, claim redemption is a separate retryable transition. The authenticated session remains authoritative while the exact journey subject is held only for the pending claim. A transient failed redemption must leave the HTTP-only pending-claim cookie, OAuth claim marker and journey-scoped local content unchanged; it must exit the loading state and offer one bounded `Try saving progress again` action. Only a successful server response may consume the claim, award XP or migrate the exact local journey namespace. Server errors distinguish a missing/unavailable pending claim from an unavailable published Programme definition using stable non-narrative codes; the interface renders one generic safe retry message rather than server or architecture detail. The transaction, one-use claim update and existing unique reward/event keys make retry exactly-once.

The authenticated Better Auth user is the sole interactive Programme subject as soon as the authentication transition resolves. A pending claim may retain the exact old journey pointer, OAuth marker and local namespace only as dormant claim-recovery inputs; they never select Mission write routes or Dashboard ownership after authentication. Access authority moves to the user without moving narrative. Successful redemption atomically records the approved Mission 01 completion and reward, retires the anonymous session, then permits the exact local journey namespace to migrate. A terminal missing, expired or conflicting claim clears only the client claim marker and resolves to the truthful authenticated zero-progress Dashboard; the old `REGISTRATION_REQUIRED` session and its local narrative remain isolated and are not edited, completed, copied or presented as saved progress. A transient Programme-definition/service failure may retain an explicit claim retry, but the retry still runs as the authenticated user and exposes no anonymous Mission editing path.

An authenticated zero-progress user starts Mission 01 through dedicated Better Auth-owned Mission 01 draft/completion routes. Those routes create or resolve only that user's Programme enrollment, persist the bounded non-narrative task-state/continuity markers, and award the existing `+60 XP` only after all eight approved tasks are complete. They never read the anonymous Programme cookie. Anonymous visitors continue to use the existing anonymous-session and one-time claim routes. Client route selection follows the settled Better Auth subject, so an authenticated Dashboard can never show Mission 01 fresh while writes address a stale anonymous session.
