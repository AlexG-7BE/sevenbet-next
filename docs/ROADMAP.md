...
| Supporting track | Frontend Migration Audit and P0 Plan | Audit complete; FE-MIG-01 Public Shell is merged in `ceee4e7` and FE-MIG-02 Home responsive parity is merged in `bef86bd`. FE-MIG-03 10 Steps is implemented in [PR #17](https://github.com/AlexG-7BE/sevenbet-next/pull/17), including the bounded post-Mission-04 availability guard and corrected pending-reward copy, and is approved pending squash merge; FE-MIG-04 must not start in this package. `/self-check` and `/tools/budget-calculator` remain P0 but launch-blocked as `P0_REDESIGN_REQUIRED` under FE-SAFETY-01; these migrations do not approve their current mechanics or result-screen commercial-navigation presentation. | 2026-08-05 |
