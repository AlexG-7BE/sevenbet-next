...
- **LAUNCH-POLISH-01:** **CURRENT MAIN INTEGRATED / LOCAL GATES PASS / VERIFIED RESEND DOMAIN / PREVIEW ACTIVATION PENDING.** Normal merge commit `0d2e67e6415566459c8c79bd2d6e6f6c2a59e67d` integrates current main and preserves PR #69 and PR #70. RFC-028 governs the resilient public 404/error hierarchy, `/contact`, `POST /api/contact`, isolated direct-HTTPS Resend adapter, privacy/retention delta, bounded link audit and WAF activation contract. Root quality, legal, Agent Core, build, 742-file secret scan, 71-route link audit, non-database browser, 19-migration fresh-database, PostgreSQL runtime and 9/9 database-backed browser gates pass. Founder Office override dated 2026-08-13 cancelled the former mail-DNS date gate and changed the visible sender to `B4GAMBLE Website <website@b4gamble.com>`. Resend is authenticated on the Free plan and the single `b4gamble.com` domain is verified/sending-enabled; authoritative Cloudflare DNS confirms preserved Google Workspace MX/SPF/DKIM, web records and DNSSEC plus verified Resend DKIM and technical `send.b4gamble.com` Return-Path MX/SPF. Existing Vercel Pro supports the exact fixed-window/IP/429 rule at `$0.50` per 1,000,000 allowed requests after its included allowance, but the rule remains unapplied. No Contact credential/environment activation, Preview/Production send, Production data or runtime flag has changed yet.
...
- **Detected provider/DNS readiness:** the Resend account is Free; the single `b4gamble.com` domain is verified and sending-enabled; `resend._domainkey` DKIM and technical `send.b4gamble.com` Return-Path MX/SPF are verified. Google Workspace root mail authority, Workspace DKIM, web DNS and DNSSEC remain present. The visible sender contract is `B4GAMBLE Website <website@b4gamble.com>`.
- **External activation not detected yet:** no Contact API credential/environment activation, real delivery smoke, Production WAF rule or DMARC mutation has occurred. The documented WAF target is `POST /api/contact`, fixed window, IP key, five requests per ten minutes, `429`.
...
- **Detected:** current main `3416a72e469f31dcba06188847e7dd1716f36ae4` integrated; merged RFC-022/RFC-023/RFC-025 Programme AI implementation, feature-on Missions 02–10 and exact `715 XP` clean-path policy plus successful controlled live Preview validation; distinct Preview/Production resource IDs and connection fingerprints; Starter workspace scope; completed Production and Preview snapshots with seven-day retention metadata; a passed Preview logical restore; a passed, fully deleted provider-native Preview restore target with exact managed canary parity; the PR #69-merged RFC-027 isolated operational-agent foundation with 40/40 no-key tests and one Founder-reviewed neutral Luna connectivity smoke (`COMPLETED` / `PASS`, 1,157 input + 275 output tokens, `$0.002807` upper bound); the PR #70 Partner Intelligence final exact-head fixed-corpus result of `8 PASS / 0 REVIEW / 0 FAIL`; and the RFC-028 Contact/error implementation on the current review branch. The committed Partner Intelligence corpus and tested boundaries have complete human-reviewed quality evidence; full agent-set quality is not established.
...
- Keep Contact delivery disabled until RFC-028's Founder, provider, privacy, mail-DNS, Preview-smoke and WAF gates pass. Account/Programme delivery remains a separate open decision; actual reminders require COMMS-REMINDER-01 and an appropriate preference/opt-out decision.
...
LEGAL-IMPL-01, AUTH-COMMS-01, BRAND-CUTOVER-01, AUTH-HARDEN-01, GOOGLE-OAUTH-ACTIVATE-01, PROGRAM-AI-IMPL-01A, PROGRAM-AI-IMPL-01B, AGENT-CORE-01, PARTNER-INTEL-EVAL-01 and the successfully Preview-validated PROGRAM-AI-ACTIVATE-01 are on current main. RFC-025’s feature-on Missions 02–10 path retains exact `715 XP` clean-path truth. AGENT-CORE-01 remains an isolated internal package with one passed neutral connectivity smoke and a passed fixed eight-case Partner Intelligence corpus; quality evidence for the other seven specialists remains pending. LAUNCH-POLISH-01 is implemented only on its unmerged review branch: the public Contact/error code exists there, but external Contact delivery, mail DNS and WAF activation remain deferred. Production Google OAuth and PROGRAM-AI providers remain off. RECOVERY-01 is `MANAGED RESTORE DRILL COMPLETE`; the closure used Preview only and left Production read-only. None of this enables reminders, mutates Production data, activates commercial traffic or makes B4GAMBLE GB launch-ready.
