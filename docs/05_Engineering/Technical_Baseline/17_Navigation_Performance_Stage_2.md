...
- **INFERRED:** On an unusually slow commercial-state read, Suspense replaces the fail-closed Header fallback after hydration, so an already-open mobile menu could reset. This was not observed in delayed-route coverage and does not change data or authority, but it remains a post-release observation point.
