...
| `LIFECYCLE_EMAIL_DELIVERY_ENABLED` | Sensitive server-only kill switch | Exact Production-only `true` is necessary but not sufficient; keep false while the sender invocation/transfer hold remains | Founder Office plus privacy/security and repository maintainer |
