...
🟨 Tilt-Locked responsive public shell — Figma family `492:2268` completes the responsive Header (`289:43`), Footer (`488:100`) and generic Availability Notice (`489:70`) across 1,440 desktop, 1,280 laptop, 390 × 844 mobile and 375 × 667 small-mobile representatives. The Header remains visually approved and the shell remains the shared visual/navigation source of truth. The Footer was revised on 2026-08-05 with a compact `Open Help` link and is review-ready pending visual confirmation; live market authority and frontend parity remain pending

🟨 Tilt-Locked `/10-steps` screen family — desktop family `502:2238` and mobile family `502:2412` cover full signed-out 1,440, 1,280 and 390 layouts plus 390×844 returning signed-in and 375×667 signed-out first folds. The family reuses the approved Public Header and revised Footer, removes the standalone Help panel, passes the revised visual/structural QA and is review-ready for visual re-approval; frontend parity is not implemented
...
**Detected and partly approved, revised 2026-08-05:** the active Figma mobile page contains the responsive Public Shell family (`492:2268`). The Header set (`289:43`) has eight desktop/mobile, theme, account and menu variants; the Footer set (`488:100`) has desktop/mobile variants; and the generic Availability Notice set (`489:70`) has desktop/mobile unknown/unavailable variants. Representative screens cover 1,440 (`492:2283`), 1,280 (`494:116`), three 390 × 844 states (`493:57`, `493:78`, `493:115`) and 375 × 667 (`493:131`). The Header remains visually approved. Footer components `289:137` and `488:69` now include 44px `Open Help` link rows `514:38` and `514:41` to `https://sevenbet-next.vercel.app/responsible-gambling`; all audited Footer instances inherit the link. The Footer revision is review-ready pending visual confirmation. This status does not claim live jurisdiction detection or frontend parity.

**Detected and review-ready, revised 2026-08-05:** `/10-steps` is designed in Figma as desktop family `502:2238` and mobile family `502:2412`, with full signed-out 1,440 (`502:2240`), 1,280 (`502:2241`) and 390 (`502:2414`) landings, a 390 × 844 returning signed-in first fold (`502:2415`) and a 375 × 667 signed-out first fold (`502:2416`). The previously approved family now removes the standalone Help section so the final `Start Mission 01` action flows directly into the shared Footer `CONTROL & SUPPORT` link. It retains 64 linked instances across the desktop/mobile families, has no remaining standalone Help instance or matching legacy text on active screen pages, and has no direct-child overflow on the audited full frames. Family/frame node IDs are unchanged; visual re-approval is pending. The current frontend remains legacy/stale, still contains the standalone Help block and displays `+20 XP`; no application code was changed.
