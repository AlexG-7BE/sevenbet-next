...
- **Detected:** Home runs the handoff reveal, floating-photo depth, sticky story stack, chameleon navigation and read progress. RFC-035 returns wheel/trackpad/keyboard/touch scroll to the browser, limits Home RAF work to pending scroll or spring settling and retains fail-visible no-JavaScript, missing-IntersectionObserver and reduced-motion paths. Its Founder correction uses CSS-only `y mandatory` on fine pointer and `y proximity` on coarse pointer with normal stops at the nine major narrative compositions; prototype percentage markers do not snap and reduced motion disables fixation.
