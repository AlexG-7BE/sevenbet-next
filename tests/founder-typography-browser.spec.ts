...
async function installDashboard(page: Page) {
  ...
  const missions = programmeMissionTitles.map((title, index) => ({ missionNumber: index + 1, title, status: index < 3 ? "completed" : index === 3 ? "current" : "locked", actionsCompleted: index === 0 ? 2 : index < 3 ? 3 : 0, actionsTotal: index === 0 ? 2 : 3, xpEarnedHere: index === 0 ? 40 : index < 3 ? 75 : 0, completionBonus: index === 0 ? 0 : 25 }));
  await page.route("**/api/program/program-ai/home", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ home: { totalXp: 190, activeDays: 18, currentStreak: 12, achievements: [{ slug: "first-plan", title: "First Plan", state: "earned", awardedAt: now }, { slug: "boundary-built", title: "Boundary Built", state: "locked", awardedAt: null }], currentMission: 4, primaryAction: "start-mission", engagementDayBucket: "day_30_plus", currentAction: "choose_boundary", startingPoint: { startingPoint: "Autopilot sessions after work that grow by the weekend.", desiredChange: "Build more control.", broadContext: "NOT_SPECIFIED", continuationCue: "Pick one boundary." }, missions, reviews: [{ milestone: "first", unlockMission: 3, title: "First Personal Review", maxWords: 250, status: "available" }, { milestone: "mid", unlockMission: 6, title: "Mid-Programme Personal Review", maxWords: 300, status: "locked" }, { milestone: "full", unlockMission: 10, title: "Full Programme Personal Review", maxWords: 450, status: "locked" }], nextReview: { milestone: "mid", unlockMission: 6, title: "Mid-Programme Personal Review", xpRemaining: 225, missionsRemaining: 3 }, discoveryLinks: [{ href: "/casinos", label: "Compare casinos" }, { href: "/bonuses", label: "Bonuses" }, { href: "/best-offers", label: "Best offers" }] } }) }));
}
