...
**DETECTED:** `PARTNER-OPS-WORK-BRIDGE-01` adds one separately authorised remote MCP adapter at `/api/mcp/commercial`. It uses OAuth 2.1 authorization-code flow with PKCE S256, ChatGPT-compatible dynamic public-client registration, explicit staff consent, `affiliate.manage`, the scopes `commercial:read` and `commercial:safe_write`, 15-minute opaque access tokens, rotating 30-day refresh tokens when `offline_access` is granted, exact resource/client binding, revocation, and MCP-specific rate limits. It is disabled unless `COMMERCIAL_MCP_ENABLED=true`; failure is isolated from Admin, consumer auth, Programme and ordinary CRM operation.
