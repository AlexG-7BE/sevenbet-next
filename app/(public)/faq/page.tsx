...
export default async function FAQPage() {
  ...
  return <>
    ...
    <article className={styles.page}><header><div><p><span aria-hidden="true" />{messages.eyebrow}</p><h1>{messages.titleLead} <em>{messages.titleEmphasis}</em></h1></div></header><div className={styles.paint} /><main>{messages.groups.map((group, groupIndex)=><section key={group.title}><h2>{group.title}</h2><div>{group.items.map(([question, answer], index)=><details key={question} open={groupIndex===0&&index===0}><summary>{question}<span aria-hidden="true">+</span></summary><p>{answer}</p></details>)}</div></section>)}<aside className={styles.contactRail}><span>{messages.railPrompt}</span><Link href="/contact">{messages.contactLink}</Link><Link href="/methodology">{messages.methodologyLink}</Link><Link href="/affiliate-disclosure">{messages.affiliateLink}</Link></aside></main></article>
  </>;
}
