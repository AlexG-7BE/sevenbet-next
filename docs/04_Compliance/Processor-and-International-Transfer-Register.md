...
| Account and Programme email delivery | Founder-approved Resend processing for transactional/account, welcome, consented Programme reminder and consented marketing purposes; Production delivery remains disabled | Minimum recipient, approved rendered message, sender/reply-to and opaque correlation data only; no Programme answers, vulnerability data, analytics history, affiliate tokens, browsing history, raw IP or auth/password token | **Detected:** RFC-046 Production ledgers and adapter, PR #271 protected-cron worker deployment, live domain/DKIM/SPF, sending-only credentials, exact five-event signed webhook, Production sender configuration and unsigned-event rejection; live delivery and six-case acceptance remain pending | Founder authority was granted 2026-09-12. Exact agreement acceptance/entity, location, retention, subprocessors, UK transfer mechanism and full activation evidence remain **open**. |
...
- No advertising SDK, affiliate pixel/postback processor, APM provider,
  ...
  Resend lifecycle processing is approved but still disabled pending bounded
  activation. Google identity code does not add a Google client SDK or mailbox
