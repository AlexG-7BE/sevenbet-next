...
| 2 | RFC-002 / RFC-008 Programme foundation | **Implemented through Mission 04; release gates remain** | Server-owned progress/rewards plus RFC-017 local-first raw narrative. Missions 05–10 remain separate scope. |
...
| 17 | GOOGLE-OAUTH-ACTIVATE-01 | **final subject-transition correction candidate — Preview E2E pending** | RFC-021; one consolidated access screen, current purpose-separated server HMAC authority bound to an exact journey/original 60-minute lifetime, bounded five-minute client issuance-skew allowance with unchanged strict server verification, session-owned Programme home/header state, truthful zero-progress Dashboard, user-owned authenticated Mission 01 writes, terminal-claim recovery with isolated anonymous state, and an exact Preview deployment-host → stable-branch-host 307 before rendering/auth. Better Auth remains stable-origin-only; static legal headers alone cannot authorize account creation. No schema, dependency, credential, commercial or Production mutation. |
| 18 | External Google OAuth configuration | **Founder Preview reconfiguration required after correction review** | Register the correction branch's exact stable Preview origin/callback in the Preview-only Web client, scope Preview credentials to that branch and complete synthetic real-provider E2E. Production activation remains separate. |
...
1. Review the GOOGLE-OAUTH-ACTIVATE-01 exact-head correction and its automated Preview evidence.
2. Re-scope the Preview-only Google client and credentials to the correction branch's exact stable alias, then run real-provider synthetic E2E; keep Production untouched.
