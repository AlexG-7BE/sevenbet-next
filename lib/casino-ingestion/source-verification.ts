...
function isMutableResearchProvenance(sourcePath: string) {
  return sourcePath.replaceAll("\\", "/").startsWith("research_staging/");
}
...
export async function verifyCasinoIngestionSources(bundle: CasinoIngestionBundle, sourceRoot: string): Promise<SourceVerificationResult> {
  ...
  for (const source of bundle.sourceFiles) {
    ...
    if (!(await stat(absolutePath)).isFile()) throw new Error(`Source must be a regular file: ${source.path}`);
    const digest = createHash("sha256").update(await readFile(absolutePath)).digest("hex");
    if (!isMutableResearchProvenance(source.path) && digest !== source.sha256) {
      throw new Error(`Frozen source checksum mismatch: ${source.path}`);
    }
