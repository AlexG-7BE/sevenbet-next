...
| Product decision | RFC-012 — Temporary Production Synthetic Casino Dataset | Implemented in [PR #20](https://github.com/AlexG-7BE/sevenbet-next/pull/20) and deployed to production: five clearly fictional `demo-*` records use the current schema/publication flow, exact-ID cleanup is available, responsive smoke passes, and no separate Demo infrastructure or external gambling destination exists | 2026-08-06 |
...
| Supporting track | Frontend Migration Audit and P0 Plan | Audit complete; FE-MIG-01 Public Shell is merged in `ceee4e7`, FE-MIG-02 Home parity in `bef86bd`, and FE-MIG-03 10 Steps in `d85146e`. FE-MIG-04 Casinos catalogue and filters is implemented in [PR #18](https://github.com/AlexG-7BE/sevenbet-next/pull/18) with SSR discovery, URL-authoritative controls, governed visit actions, responsive states and bounded data-honesty fixes backed by deterministic full/sparse/preview rendering tests; final checks/merge remain pending. FE-MIG-05 must not start in this package. `/self-check` and `/tools/budget-calculator` remain launch-blocked as `P0_REDESIGN_REQUIRED` under FE-SAFETY-01. | 2026-08-05 |
