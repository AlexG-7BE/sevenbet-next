...
| `CONTACT_EMAIL_FROM`, `CONTACT_EMAIL_TO` | Server-only sender/recipient configuration | Exact approved visible From identity `B4GAMBLE Website <website@b4gamble.com>` and existing `support@b4gamble.com` mailbox only | Founder Office/communications owner; verified `b4gamble.com` sending authority required before activation; provider Return-Path records may remain under `send.b4gamble.com` |
