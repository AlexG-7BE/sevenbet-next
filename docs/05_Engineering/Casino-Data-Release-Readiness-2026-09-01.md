...
- **PROPOSED — one-time operator:** branch `release/casino-market-0025-one-time-operator` is based on exact PR #113 head `830d3398fe68a34d0f8f92138d01c3e4b8774d95`. It adds a default-dry-run, exact-commit/checksum/identity-bound local operator CLI and disposable failure matrix. It has not accessed Production, run a Production dry run, executed 0025, deployed an application, or been approved for merge.
