...
import { MARKET_ACTIVATION_CONTROLLER_VERSION, safeActivationDestination, type MarketActivationIntentInput } from "../lib/market-activation/contract";
...
async function migrationChecksum() {
  return sha256(await readFile(`prisma/migrations/${MIGRATION}/migration.sql`));
}
...
async function assertMigrationApplied() {
  const expected = await migrationChecksum();
  const rows = (await completedMigrations()).filter((row) => row.migration_name === MIGRATION && row.finished_at && !row.rolled_back_at);
  if (rows.length !== 1 || rows[0].checksum !== expected) throw new Error("MARKET_ACTIVATION_MIGRATION_NOT_EXACTLY_ONCE_OR_CHECKSUM_MISMATCHED");
  return { migration: MIGRATION, checksum: expected, attempts: (await completedMigrations()).filter((row) => row.migration_name === MIGRATION).length };
}
...
async function backfill(now = new Date()) {
  ...
  for (const route of legacy) {
    results.push(await marketActivationController.activateCasinoInGeo({
      ...
      actorId: RELEASE,
      origin: "BACKFILL",
      reason: "Preserve a currently Production-eligible exact-market route during canonical-authority expansion.",
      sourceReferences: ["LEGACY_PARTNER_ROUTE_PROJECTION", route.tracking.countryAuthority?.productionEligibilityEvidence ?? FOUNDER_SOURCE],
...
async function shadow(now = new Date()) {
  ...
  for (const profile of profiles) {
    ...
    comparisons.push({ casinoId: profile.casinoId, countryCode: profile.countryCode, legacyActive, canonicalActive, match: legacyActive === canonicalActive });
  }
  return {
    release: RELEASE,
    comparisons,
...
async function verify() {
  ...
  for (const fixture of golden) {
    ...
    const ok = activation?.desiredState === "ACTIVE" && activation.status === "ACTIVE"
      && activation.routeVerificationStatus === "HEALTHY" && Boolean(activation.routeLastCheckedAt && activation.routeFinalHost)
      && activation.redirectSlug?.slug === fixture.redirectSlug && Boolean(route)
      && (activation.casino._count.mediaAssignments + activation.casino._count.partnerHostedAssignments) > 0;
    outcomes.push({
      ...fixture,
      ok,
      ...
      mediaAssignments: activation ? activation.casino._count.mediaAssignments + activation.casino._count.partnerHostedAssignments : 0,
    });
  }
  ...
  if (outcomes.some((entry) => !entry.ok) || !negativeOk) throw new Error("MARKET_ACTIVATION_GOLDEN_VERIFICATION_FAILED");
  return { release: RELEASE, verified: true, outcomes, betssonCl: { ok: negativeOk, desiredState: betssonCl?.desiredState ?? null, status: betssonCl?.status ?? null }, diagnostics };
}
