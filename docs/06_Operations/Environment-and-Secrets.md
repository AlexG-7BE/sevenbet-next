...
| `CONTACT_EMAIL_FROM`, `CONTACT_EMAIL_TO` | Server-only sender/recipient configuration | Exact approved visible From identity `B4GAMBLE <info@b4gamble.com>` and existing `support@b4gamble.com` mailbox only | Founder Office/communications owner; `info@b4gamble.com` must be a verified Google Workspace alias to the support mailbox and verified `b4gamble.com` Resend authority is required before activation; provider Return-Path records may remain under `send.b4gamble.com` |
