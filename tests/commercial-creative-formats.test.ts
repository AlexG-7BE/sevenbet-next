...
function profileCasino(renderingMode: "CONTAIN" | "COVER" | "COMPOSED" = "CONTAIN"): PublicCasinoDTO {
  ...
  return {
    ...
    bonuses: [bonus],
    offerPresentation: { selectedOffer: bonus, relation: "EXACT", sourceCountryCode: "GB", presentationCountryCode: "GB", currentMarketVerified: true },
...
test("review heroes contain no promotional media while governed CTA authority stays separate", async () => {
  const require = createRequire(import.meta.url);
  require.extensions[".css"] = () => undefined;
  (globalThis as typeof globalThis & { React: typeof React }).React = React;
  ...
  for (const renderingMode of ["CONTAIN", "COVER", "COMPOSED"] as const) {
    ...
    assert.doesNotMatch(html, /data-media-mode|data-media-source|data-presentation-family|skol-300x250/);
  ...
  blockedCasino.offerPresentation = { ...blockedCasino.offerPresentation!, selectedOffer: blockedCasino.bonuses[0] };
  ...
  assert.doesNotMatch(blocked, /data-media-mode|data-media-source|data-presentation-family|skol-300x250/);
  ...
  assert.doesNotMatch(fallback, /data-media-mode|data-media-source|data-presentation-family/);
  ...
  assert.doesNotMatch(brandArtHtml, /data-media-mode|data-media-source|data-presentation-family/);
  ...
  assert.doesNotMatch(logoOfferHtml, /data-media-mode|data-media-source|data-presentation-family/);
...
test("active Admin and public surfaces keep CTA paths while promotional placement tooling is retired", () => {
  ...
  const identity = readFileSync("components/commercial-media/OperatorIdentityPanel.tsx", "utf8");
  const profile = readFileSync("components/casino-profile/CasinoProfile.tsx", "utf8");
  const assignmentRoute = readFileSync("app/api/admin/media/assignments/route.ts", "utf8");
  assert.match(editor, /Promotional placement media retired/);
  assert.match(editor, /MarketActivation/);
  assert.match(editor, /Canonical operator logos/);
  assert.doesNotMatch(editor, /assessCommercialCreative|Upload and assign|Remove assignment|Deactivate assignment/);
  assert.match(assignmentRoute, /retiredMediaResponse/);
  assert.doesNotMatch(assignmentRoute, /mediaAssignmentService|assignMedia|unassignMedia/);
  assert.match(selector, /image\/gif/);
  assert.match(identity, /operator logo only, never promotional artwork/i);
  assert.match(identity, /ResponsivePlacementImage/);
  ...
  assert.doesNotMatch(profile, /data-media-mode|data-media-source|data-presentation-family/);
