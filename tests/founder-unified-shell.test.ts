...
test("the measured Home and Learn grid owns the public geometry tokens", () => {
  ...
  assert.match(globals, /html:has\(body > \[data-public-shell="header"\]\):not\(:has\(\[data-handoff-page="home"\]\)\),\s*html:has\(\[data-protected-help-shell\]\)\s*\{\s*scrollbar-width:\s*none/);
  assert.match(globals, /html:has\(body > \[data-public-shell="header"\]\):not\(:has\(\[data-handoff-page="home"\]\)\)::\-webkit-scrollbar/);
  assert.match(globals, /body:has\(\[data-protected-help-shell\]\)::\-webkit-scrollbar/);
});
