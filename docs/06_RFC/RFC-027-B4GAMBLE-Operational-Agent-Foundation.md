...
## 18. `PARTNER-OPS-WORK-BRIDGE-02` Better Auth 1.7 amendment

Founder instruction on 2026-08-20 authorises a coordinated dependency, schema and regression upgrade of the section 17 bridge without broadening its Agent, tool or commercial authority.

1. `better-auth`, `@better-auth/core` and `@better-auth/oauth-provider` move together from `1.6.30` to stable `1.7.1`. No beta/RC, legacy embedded MCP plugin, generic `@better-auth/mcp`, CIMD or JWT surface is added by this amendment.
2. The provider-owned protected-resource architecture replaces `validAudiences`. Exactly one resource remains authorised: the environment-bounded `/api/mcp/commercial` identifier. Authorization codes, access tokens, refresh tokens and consent retain provider-owned resource arrays; each DCR client has one client-resource relation; the application still checks the exact resource and client metadata on every use.
3. Account identity adds the Better Auth 1.7 trusted issuer key. Existing credential accounts deterministically use `local:credential`; Google accounts use the provider-declared `https://accounts.google.com`. Unsupported providers, credential identity mismatches, incomplete backfills and issuer/account collisions fail the migration rather than receiving invented values.
4. Bounded public DCR is retained because current OpenAI hosts continue to support it and it is the already-reviewed path. Registration remains allowlisted, public, `web`, secretless and zero-authority. Client-credentials grants remain unavailable and client-credentials scopes remain empty. CIMD may be considered only under a later separate review.
5. Provider revocation, session linkage, access/refresh expiry, rotation and replay handling remain authoritative. Reusable access/refresh/code values remain protected by the provider storage hook; application logs and Commercial audit metadata contain no credential values.
