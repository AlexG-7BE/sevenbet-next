...
- **Implemented / read-only retry separately authorised / migration not executed:** [Casino Market 0025 Production Build Probe](Casino-Market-0025-Production-Build-Probe.md) documents mutually exclusive attended build modes. The probe uses the verified direct administrative binding in a bounded repeatable-read, PostgreSQL-read-only transaction and intentionally fails before runtime promotion. A separate dormant exact-authority executor can apply only final checksum-pinned 0025 after a new Founder GO; no CI, Preview, Git, normal Production build, or persistent Vercel variable invokes it, and its capability confers no current Production mutation authority.
