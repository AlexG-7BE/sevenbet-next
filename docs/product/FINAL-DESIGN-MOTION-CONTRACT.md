...
RFC-035 supersedes the original Home wheel-tween and Home rise-timing rows only. Its 2026-08-19 Founder correction preserves browser ownership while applying mandatory fixation on fine pointer and proximity fixation on coarse pointer at deliberate major compositions; it does not restore input interception.
...
| Home | Major narrative compositions | Native wheel, trackpad, keyboard or touch scroll ends near an approved target | User-controlled physical scroll position | Browser settles to a valid Hero, Recognition, plan, chapter, evidence, trust or final-composition boundary | Browser-native `y mandatory` fine pointer / `y proximity` coarse pointer; `scroll-snap-stop: normal` | Browser-native | Fine pointer stronger fixation; coarse pointer less aggressive; stronger input may skip targets | Snap disabled; native direct scroll | Home handoff CSS; no JS scroll controller |
...
- Home fixation is CSS-only: mandatory on fine pointer, proximity on coarse pointer, disabled for reduced motion. It targets major compositions rather than prototype percentage markers, uses normal stop behaviour and never cancels or delays wheel, touch or keyboard input.
