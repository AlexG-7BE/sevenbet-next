...
export const PR6_TARGET_TABLES = [
  { model: "OauthClient", table: "oauthClient", baselineCount: 11 },
  { model: "OauthResource", table: "oauthResource", baselineCount: 5 },
  { model: "OauthClientResource", table: "oauthClientResource", baselineCount: 11 },
  { model: "OauthRefreshToken", table: "oauthRefreshToken", baselineCount: 230 },
  { model: "OauthAccessToken", table: "oauthAccessToken", baselineCount: 240 },
  { model: "OauthConsent", table: "oauthConsent", baselineCount: 8 },
  { model: "OauthClientAssertion", table: "oauthClientAssertion", baselineCount: 0 },
  {
...
export function expectedPr6TotalRows() {
  return PR6_TARGET_TABLES.reduce((total, target) => total + target.baselineCount, 0);
}
...
export function assessPr6Projection(evidence: Pr6ProjectionEvidence) {
  ...
  if (evidence.transactionReadOnly !== "on") conflicts.push("TRANSACTION_NOT_READ_ONLY");
  ...
  for (const target of PR6_TARGET_TABLES) {
    ...
    if (count !== target.baselineCount) {
      conflicts.push(`TARGET_ROW_COUNT_DRIFT:${target.table}:${String(count)}!=${target.baselineCount}`);
    }
  }
  ...
  if (canonicalJson(actualForeignKeys) !== canonicalJson(expectedForeignKeys)) {
    conflicts.push("FOREIGN_KEY_GRAPH_DRIFT");
  }
  ...
  if (canonicalJson(actualTriggers) !== canonicalJson(expectedTriggers)) {
    conflicts.push("TRIGGER_GRAPH_DRIFT");
  }
  if (evidence.triggers.some((trigger) => trigger.enabled === "DISABLED")) {
    conflicts.push("TARGET_TRIGGER_DISABLED_DRIFT");
  }
  ...
  if (canonicalJson(actualFunctions) !== canonicalJson(expectedFunctions)) {
    conflicts.push("FUNCTION_INVENTORY_DRIFT");
  }
  ...
  if (canonicalJson(actualFunctionDependants) !== canonicalJson(expectedFunctionDependants)) {
    conflicts.push("FUNCTION_DEPENDANT_GRAPH_DRIFT");
  }
  ...
  if (evidence.activeRuntimeConsumerFiles.length) conflicts.push("ACTIVE_RUNTIME_CONSUMER_FOUND");
  ...
  const totalRowsScheduledForDeletion = Object.values(targetRowCounts).every((value) => typeof value === "number")
    ...
    : null;
  const reviewedPlan = {
    schemaVersion: "commercial-core-pr6-reviewed-plan.v1",
    head: evidence.head,
    migrationSqlSha256: evidence.migration.sqlSha256,
    targets: {
      tables: PR6_TARGET_TABLES.map(({ model, table }) => ({ model, table })),
  ...
  return {
    conflicts: uniqueConflicts,
    readyToApply: uniqueConflicts.length === 0,
    reviewedPlan,
