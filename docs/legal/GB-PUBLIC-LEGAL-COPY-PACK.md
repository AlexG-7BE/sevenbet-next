...
The GB launch uses only B4GAMBLE storage needed for requested features, authentication, security, access confirmation, same-tab comparison selection and temporary Programme continuity. It does not run non-essential product analytics or session replay. A published vetted partner-hosted creative may make a no-referrer media/impression request to Superfly or Bannerflow; the exact evaluated fixtures set no cookie and made no observed browser-storage write. Revalidate new provider behavior and apply the existing user-choice boundary before loading if future evidence shows non-essential storage or access that requires it.
