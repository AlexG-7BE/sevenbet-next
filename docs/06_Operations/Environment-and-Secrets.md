...
| `OPENAI_API_KEY` | Secret | Server-only OpenAI API authentication after database isolation; consumed by the gated Programme adapters and, only when separately enabled, RFC-052 Learn image generation. Current Production presence must be verified without revealing the value; never client-exposed, printed or documented as a value. | Founder Office/OpenAI account owner; repository maintainer technical consumer |
