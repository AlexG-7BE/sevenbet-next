...
function EvidenceCard({ mission }: { mission: 1 | 2 | 3 | 4 }) {
  ...
  return (
    <article className={styles.evidenceCard}>
      ...
      <small>{isFirst
        ? "B4GAMBLE adapts the idea as education. This mission is not CBT, assessment or treatment."
        : isThird
          ? "Research does not establish one universal urge pattern. This is education, not diagnosis, CBT or treatment."
        : isFourth
          ? "Evidence for voluntary limit tools is mixed. This B4GAMBLE mission has not been clinically evaluated."
        : "B4GAMBLE adapts goal clarification as self-directed education. It is not motivational interviewing or treatment."}</small>
...
function MissionOneScreen({
  ...
  return (
    <MissionShell mission={1} step={step} xp={0}>
      <section className={styles.resultLayout}><div><div className={styles.titleBlock}><span>REVIEW · ABOUT 2 MIN</span><h1>Your Moment Map is ready.</h1><p>Check the local result before deciding whether to save completion and XP to a B4GAMBLE account.</p></div><ArtifactCard eyebrow="MISSION 01 RESULT · LOCAL" title="Your Moment Map" body={momentSummary(map)} footer={`Notice rule: ${map.noticeRule}`} dark /></div><aside className={styles.rewardColumn}><Recognition label="PROGRESS AFTER ACCOUNT" value="+60 XP" note="Awarded after account creation." /><ul><li>Your five-part Moment Map stays in this tab</li><li>Your personal notice rule stays in this tab</li><li>Mission 01 completion and 60 XP save to the account</li></ul></aside><PhotoTheatre compact image={PEOPLE.planning} eyebrow="MISSION 01 · RESULT READY" title="Useful work, locally held." note="Only completion, reward and neutral continuity become persistent." /></section>
...
function GoogleMark() {
  return (
    <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24">
      ...
      <path d="M6.4 14a6 6 0 0 1 0-3.9V7.4H3A10 10 0 0 0 3 16.7L6.4 14Z" fill="#FBBC05" />
        ...
        <form onSubmit={submit}>
          ...
          {googleAvailable ? (
            <>
              ...
              <p className={styles.googleBoundary}>Google provides account identity only. It is not age verification or marketing consent.</p>
    ...
    return <MissionShell mission={3} step={step} xp={140}>
      <section className={styles.learningGrid}>
        ...
        <div className={styles.stack}>
          ...
          <Choice active={learning.evidenceReviewed} title="I reviewed the evidence note and limitation" description="The complete B4GAMBLE Programme has not been clinically evaluated." onClick={() => setLearning({ ...learning, evidenceReviewed: true })} />
        </div>
      </section>
...
export function ActiveControlProgramme({ googleAvailable = false }: { googleAvailable?: boolean }) {
  ...
  useEffect(() => {
    ...
    const retryJourney = authAction === "google-error" && nextSubject.kind === "journey"
      ? readProgrammeOAuthClaimMarker(window.sessionStorage)
      : null;
    if (retryJourney && programmeSubjectsEqual(retryJourney, nextSubject)) {
      claimedJourneySubject.current = retryJourney;
    }
    if (authAction === "google-error") {
      setReturningSignIn(!retryJourney);
  ...
  async function handleGoogle(input: { mode: "sign-up" | "sign-in"; adultConfirmed: boolean; termsAccepted: boolean }) {
    ...
    const continuingCurrentClaim = !returningSignIn
      && activeSubject?.kind === "journey"
      && programmeSubjectsEqual(activeSubject, claimedJourneySubject.current);
    if (continuingCurrentClaim && activeSubject) {
      writeProgrammeOAuthClaimMarker(window.sessionStorage, activeSubject);
    } else {
      clearProgrammeOAuthClaimMarker(window.sessionStorage);
    }
    setClaimTransitionPending(continuingCurrentClaim);
    ...
    try {
      ...
      setClaimTransitionPending(false);
      setError(cause instanceof Error ? cause.message : "Google sign-in could not be started. Try again or use email.");
      setBusy(false);
    }
  }
