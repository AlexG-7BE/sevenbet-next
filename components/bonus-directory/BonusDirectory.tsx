...
function DemoFixtureNotice({ offer }: { offer: PublicOfferDTO }) {
  ...
  return <p className={styles.conditions}><strong>DEMONSTRATION DATA</strong> · Fictional record. Not a current GB promotion, partner offer or claimable bonus.</p>;
}
...
export function ActiveBonusFilters({ query, raw }: { query: PublicOfferQuery; raw: PublicOfferSearchParams }) {
  ...
  if (!values.length) return <p className={styles.filterSummary}>All current comparison records are shown.</p>;
...
export function BonusEducation() {
  return <>
    ...
    <section className={styles.faqSection}><div className={styles.shell}><div className={styles.faqHeading}><span>Bonus education · neutral answers</span><h2>Questions before action.</h2><p>A published bonus is not a recommendation to play. Compare the cost, restrictions and local context first.</p></div><div className={styles.faqList}><details><summary>What does wagering mean?</summary><p>It describes the turnover required under the published terms. If the value is missing, B4GAMBLE shows it as not listed instead of estimating it.</p></details><details><summary>Does a larger maximum bonus make an offer better?</summary><p>No. Deposit requirements, wagering, eligibility, expiry and withdrawal restrictions may matter more than the headline amount.</p></details><details><summary>Does an available action prove I am eligible?</summary><p>No. Country is only a comparison preference. Confirm local law, operator eligibility and the current operator terms yourself.</p></details><details><summary>How does B4GAMBLE earn money?</summary><p>Some governed outbound links may compensate B4GAMBLE. Publication and server-owned sorting do not depend on commercial availability.</p></details></div></div></section>
  </>;
}
