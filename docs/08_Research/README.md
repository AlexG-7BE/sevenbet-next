# Research

## Purpose

Stores evidence that informs product, market, user, regulatory, and technical decisions.

## What documents belong here

- User research.
- Market and competitor research.
- Regulatory research.
- Experiment findings and evidence summaries.
- [GB commercial partner shortlist, application and economics evidence](GB-Partner-Readiness-Package.md).

## When this folder should be updated

Update when new evidence is gathered or when research findings materially change an active decision.
