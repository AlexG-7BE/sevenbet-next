...
function CommercialBoundary({ result }: { result: PublicComparisonResult }) {
  ...
  return <section className={styles.commercialSection} aria-labelledby="commercial-title"><div className={styles.shell}>
    ...
    <div className={styles.commercialCards}>{result.casinos.map((casino) => <article key={casino.slug}><span>{casino.name}</span><p>{casino.action ? "Governed commercial action available." : "No governed commercial action is currently available."}</p><Link href={casino.reviewHref}>{casino.dataClassification === "DEMO_FIXTURE" ? "Fictional profile" : "Full review"}</Link>{casino.action ? <CasinoOutboundAction action={{ href: casino.action.href, label: `Visit ${casino.name}` }} className={styles.outboundAction} context={{ source: "CTA", placement: "CASINO_COMPARE" }} /> : <span aria-disabled="true" className={styles.unavailableAction}>Commercial action unavailable</span>}</article>)}</div>
  </div></section>;
}
