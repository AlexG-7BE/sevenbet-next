...
test("Public Shell keeps its approved architecture while exposing the current brand", () => {
  ...
  assert.match(publicLayout, /isCpoCommercialPreviewEnabled/);
  assert.match(publicLayout, /PREVIEW ONLY/);
  execFileSync("git", ["diff", "--quiet", "origin/main", "--", "app/design-system.css"]);
