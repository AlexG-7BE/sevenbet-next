...
const nextActions = [
  {
    number: "01",
    title: "Pause right now",
    description: "Close gambling sites and use cooling-off, operator limits or blocking tools where available.",
  },
  {
    number: "02",
    title: "Self-exclude",
    description: "Check the scope and commitment before registering with an operator or national scheme.",
  },
  {
    number: "03",
    title: "Control access",
    description: "Ask your bank about gambling blocks and review device-blocking or account tools.",
  },
  {
    number: "04",
    title: "Talk to support",
    description: "Open an independent organisation or speak to someone you trust.",
  },
] as const;
    ...
    <article className={styles.hub} data-protected-help-page="help">
      <section className={styles.hero} aria-labelledby="protected-help-title" data-help-section="hero">
        ...
        <div className={styles.heroActions}>
          <a className={styles.primaryAction} href="#next-actions">Pause and block access</a>
          <a className={styles.secondaryAction} href="#external-support">Find external support</a>
        </div>
      </section>
      ...
      <section className={styles.support} id="external-support" aria-labelledby="support-title" data-help-section="external-support">
        <p className={styles.eyebrow}>Verified UK examples</p>
        <h2 id="support-title">Independent support.</h2>
        <p className={styles.sectionIntro}>
          These destinations were checked against the providers’ official sites on 7 August 2026. Check the provider’s current coverage before relying on a service.
        </p>
        {protectedHelpResources.length > 0 ? (
          <div className={styles.resourceGrid}>
            {protectedHelpResources.map((resource) => (
              <article className={styles.resource} key={resource.name}>
                <div className={styles.resourceCopy}>
                  <p className={styles.resourceStatus}>Verified source</p>
                  <h3>{resource.name}</h3>
                  <p>{resource.description}</p>
                  <small>{resource.region} · verified 7 August 2026</small>
                </div>
                <a
          ...
          <p className={styles.unavailableResource} role="status">
            No verified external support destination is available to show here. Check current details with an official provider for your location.
          </p>
        )}
        <aside className={styles.urgentBoundary} aria-labelledby="urgent-boundary-title">
          ...
          <a
            ...
            target="_blank"
          >
            View NHS gambling help <span aria-hidden="true">↗</span>
            <span className={styles.srOnly}> (opens an external site in a new tab)</span>
          </a>
        </aside>
      </section>

      <section className={styles.programme} aria-labelledby="programme-title" data-help-section="privacy-boundary">
        <div>
          <p className={styles.eyebrow}>Optional control plan</p>
