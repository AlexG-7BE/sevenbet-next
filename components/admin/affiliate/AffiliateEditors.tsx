...
type RedirectPreviewCandidate = { trackingLinkId: string; label: string; network: string; program: string; specificity: string; priority: number; verifiedAt: string | null; chosen: boolean };

function AffiliateRedirectPanel({ offerId, casinoId, casinoBonusId, suggestedSlug }: { offerId: string; casinoId: string; casinoBonusId: string; suggestedSlug: string }) {
  const [records, setRecords] = useState<AffiliateRedirectSlugRecord[]>([]);
  const [slug, setSlug] = useState(suggestedSlug);
  const [hints, setHints] = useState({ country: "", currency: "", language: "" });
  const [preview, setPreview] = useState<{ reason: string | null; winner: RedirectPreviewCandidate | null; candidates: RedirectPreviewCandidate[] } | null>(null);
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const [copied, setCopied] = useState("");
  const copyTimer = useRef<number | null>(null);
  const load = useCallback(async () => {
    const result = await json<{ records: AffiliateRedirectSlugRecord[] }>(`/api/admin/affiliate/redirect-slugs?offerId=${encodeURIComponent(offerId)}`);
    setRecords(result.records); setError("");
  }, [offerId]);
  useEffect(() => { void load().catch((caught) => setError(caught instanceof Error ? caught.message : "Unable to load redirect slugs")); }, [load]);
  useEffect(() => { if (!slug) setSlug(suggestedSlug); }, [slug, suggestedSlug]);
  useEffect(() => () => { if (copyTimer.current) window.clearTimeout(copyTimer.current); }, []);
  async function createRedirect() {
    setSaving(true); setError("");
    try {
      await json("/api/admin/affiliate/redirect-slugs", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ slug: slugify(slug), casinoId, casinoBonusId: casinoBonusId || null, affiliateOfferId: offerId, active: true }) });
      setSlug(""); await load();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to create redirect slug"); }
    finally { setSaving(false); }
  }
  async function toggle(record: AffiliateRedirectSlugRecord) {
    setSaving(true); setError("");
    try {
      await json(`/api/admin/affiliate/redirect-slugs/${record.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ active: !record.active, expectedUpdatedAt: record.updatedAt }) });
      await load();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to update redirect slug"); }
    finally { setSaving(false); }
  }
  async function runPreview(record: AffiliateRedirectSlugRecord) {
    const params = new URLSearchParams({ slug: record.slug });
    if (hints.country) params.set("country", hints.country);
    if (hints.currency) params.set("currency", hints.currency);
    if (hints.language) params.set("language", hints.language);
    try {
      const result = await json<{ reason: string | null; winner: RedirectPreviewCandidate | null; candidates: RedirectPreviewCandidate[] }>(`/api/admin/affiliate/redirect-preview?${params}`);
      setPreview(result); setError("");
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Unable to preview redirect"); }
  }
  async function copyRedirect(record: AffiliateRedirectSlugRecord) {
    try {
      await navigator.clipboard.writeText(new URL(`/r/${record.slug}`, window.location.origin).toString());
      setCopied(record.id);
      if (copyTimer.current) window.clearTimeout(copyTimer.current);
      copyTimer.current = window.setTimeout(() => setCopied(""), 1500);
    } catch { setError("The redirect URL could not be copied. Check browser clipboard permission."); }
  }
  return <AffiliateSectionLayout badge="Phase 3.7" title="Redirect URLs" description="Stable public keys resolve server-side. No destination URL or affiliate identifier is accepted from the public request."><div className="affiliateRedirectCreate"><Field label="New redirect slug"><input onChange={(event) => setSlug(slugify(event.target.value))} placeholder="casino-welcome" value={slug} /></Field><button className="button gold" disabled={saving || slug.length < 3} onClick={() => void createRedirect()} type="button">Create redirect URL</button></div>{error && <p className="builderError" role="alert">{error}</p>}<div className="affiliateRedirectList">{records.map((record) => <Card className={!record.active ? "affiliateArchived" : ""} key={record.id}><div className="affiliateRecordHeader"><div><div className="badgeCluster"><Badge tone={record.active ? "green" : "warning"}>{record.active ? "ACTIVE" : "ARCHIVED"}</Badge><Badge>{record.revisions.length} revisions</Badge></div><strong className="affiliateRedirectUrl">/r/{record.slug}</strong><p className="muted">{record.casinoBonus?.title || "Casino-level"} · created {new Date(record.createdAt).toLocaleDateString("en-US")}</p></div><div className="builderActions"><button className="button ghost" onClick={() => void copyRedirect(record)} type="button">{copied === record.id ? "Copied" : "Copy URL"}</button><button className="button ghost" disabled={saving} onClick={() => void toggle(record)} type="button">{record.active ? "Archive" : "Restore"}</button></div></div><div className="affiliateFilters"><input aria-label="Preview country" maxLength={2} onChange={(event) => setHints((value) => ({ ...value, country: event.target.value.toUpperCase() }))} placeholder="Country" value={hints.country} /><input aria-label="Preview currency" maxLength={3} onChange={(event) => setHints((value) => ({ ...value, currency: event.target.value.toUpperCase() }))} placeholder="Currency" value={hints.currency} /><input aria-label="Preview language" onChange={(event) => setHints((value) => ({ ...value, language: event.target.value }))} placeholder="Language" value={hints.language} /><button className="button ghost" disabled={!record.active} onClick={() => void runPreview(record)} type="button">Test routing</button></div></Card>)}{!records.length && !error && <p className="muted">No redirect URLs have been created for this offer.</p>}</div>{preview && <Card tone="soft"><div className="badgeCluster">{preview.winner ? <Badge tone="green">Candidate selected</Badge> : <Badge tone="warning">No candidate</Badge>}{preview.reason && <Badge>{preview.reason.replaceAll("_", " ")}</Badge>}</div>{preview.winner ? <p><strong>{preview.winner.label}</strong><br /><span className="muted">{preview.winner.network} · {preview.winner.program} · {preview.winner.specificity} · priority {preview.winner.priority}</span></p> : <p className="muted">No redirect will be issued for these inputs.</p>}</Card>}</AffiliateSectionLayout>;
}
