...
| `RESEND_API_KEY` | Secret | Server-only authentication for the isolated Contact Resend adapter; never client-exposed, printed or committed | Founder Office/Resend owner; narrow send permission preferred |
...
| `PUBLIC_CASINO_CMS_ENABLED` | Sensitive feature flag | Local/test compatibility selector. Current main reads it directly; the unmerged audit branch forces the governed CMS path in positively identified Vercel Preview/Production runtimes. | Repository maintainer |
...
| `OPENAI_API_KEY` | Secret | Server-only OpenAI API authentication after database isolation; historically configured only for controlled Preview evidence. Current Production presence is unknown and was not inspected; never client-exposed, printed or documented as a value. | Founder Office/OpenAI account owner; repository maintainer technical consumer |
...
7. Disable affiliate redirects and public CMS writes, use local media storage, and keep Production S3, Contact/account/Programme email-delivery, webhook, analytics and affiliate credentials absent unless a separate exact activation stage authorises the specific Preview value. Preview Google OAuth may use a separate non-Production client only after its exact stable branch callback is registered; never copy the Production Google secret. Add any other sandbox credential only through separate approval.
