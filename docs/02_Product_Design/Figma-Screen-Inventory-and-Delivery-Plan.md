...
| 26 | Commercial Handoff | Desktop family `679:5238`; confirmation `679:5319`; mobile family `679:8391`; confirmation `679:8489` | **Superseded in runtime by `DIRECT-GOVERNED-OUTBOUND-01`.** Historical frames remain design history; normal commercial actions now navigate directly through `/r/{slug}` with no confirmation UI. |
