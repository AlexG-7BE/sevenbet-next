...
export function CasinoProfileUnavailable({
  ...
  return <article className={`${styles.page} ${styles.unavailablePage}`}>
    <div className={styles.unavailableShell}>
      ...
      <div className={styles.unavailableLinks}><Link className={styles.primaryAction} href={presentation ? productHref(presentation, "/casinos") : "/casinos"}>{messages?.common.browseReviews ?? "Browse published reviews"}</Link><Link className={styles.secondaryAction} href="/methodology">{messages?.common.reviewMethodology ?? "View methodology"}</Link></div>
