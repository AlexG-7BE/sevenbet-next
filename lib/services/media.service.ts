  ...
  async upload(input: UploadMediaInput) {
    ...
    try {
      ...
      const record = await this.repository.create({
        ...
        metadata: {
          ...(cleanMetadata(input.metadata) as Prisma.JsonObject | undefined),
          metadataStripped: processed.metadataStripped,
          sourceSizeBytes: original.length,
          detectedMimeType: validated.mimeType,
          detectedWidth: validated.width,
          detectedHeight: validated.height,
          animated: validated.animated,
          ...("frameCount" in validated ? { animationFrameCount: validated.frameCount } : {}),
        },
