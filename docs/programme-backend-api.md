...
Idempotency is server-authored: claim consumption is conditional; mission/progress, XP, achievement and active-day rows have database uniqueness constraints; Mission 02 completion retries return the existing Dashboard. Client idempotency keys are not trusted or required.
