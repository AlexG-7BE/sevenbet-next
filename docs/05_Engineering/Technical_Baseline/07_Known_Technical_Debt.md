...
| --- | --- | --- | --- |
| Provider recovery capability | **Detected:** both Prisma Postgres resources are Free; their live backup pages expose no snapshots. RFC-024's Preview logical backup restored successfully to a disposable local PostgreSQL 16 target, but no managed Production recovery point or provider-native restore has been proven. | Logical recoverability is proven; the RPO and seven-daily-point objectives remain unmet until Founder Office approves a managed backup path and its first isolated provider restore is verified. | High |
