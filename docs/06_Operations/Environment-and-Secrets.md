...
Reconciled on 2026-08-12 for the approved B4GAMBLE canonical-domain release contract, Preview isolation, RECOVERY-01, MVP-RUNTIME-01 and LAUNCH-POLISH-01 Contact boundary. Secret values are intentionally omitted.
...
| `CONTACT_EMAIL_DELIVERY_ENABLED` | Sensitive server-only Contact kill switch | Exact `true` enables only the RFC-027 Contact transport after complete validation; absent/default off | Founder Office plus repository maintainer |
...
| `CONTACT_EMAIL_FROM`, `CONTACT_EMAIL_TO` | Server-only sender/recipient configuration | Exact approved `send.b4gamble.com` transactional identity and existing `support@b4gamble.com` mailbox only | Founder Office/communications owner; sending subdomain verification required before activation |
