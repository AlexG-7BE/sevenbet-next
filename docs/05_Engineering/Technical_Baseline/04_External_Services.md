...
| --- | --- | --- | --- | --- | --- |
| Prisma Postgres | Detected | Redacted hosted endpoint classification plus Prisma datasource/repositories. Backup/restore capabilities are not verified. | High | `DATABASE_URL`, `DIRECT_URL`, provider aliases | Founder Office/config owner; technical consumer repository maintainer |
...
**Not detected:** payment/email/analytics/APM providers, queues, automated paging, infrastructure-as-code, or a verified provider recovery procedure. Secret values were never recorded.
