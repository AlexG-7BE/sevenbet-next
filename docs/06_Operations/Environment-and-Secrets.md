...
| `PROGRAM_AI_PROVIDER`, `PROGRAM_AI_OPENAI_MODEL`, `PROGRAM_AI_TRANSCRIPTION_MODEL` | Server-only provider configuration | Historically approved PR #64 Preview values were respectively `openai`, `gpt-5.6-terra`, `gpt-4o-transcribe`; any other configured model fails closed. Current Production provider values are unknown and were not inspected. | Repository maintainer under RFC-023 |
| `OPENAI_API_KEY` | Secret | Server-only OpenAI API authentication after database isolation; consumed by the gated Programme adapters, separately enabled RFC-052 Learn image generation and separately enabled RFC-053 managed Learn editorial sessions. Current Production presence must be verified without revealing the value; never client-exposed, printed or documented as a value. | Founder Office/OpenAI account owner; repository maintainer technical consumer |
...
| `LEARN_MCP_ENABLED`, `LEARN_MCP_ACTOR_ID` | Sensitive runtime configuration | Exact `true` enables the single `learn_apply` endpoint; UUID binds every write/revision/audit to the provisioned unlinked `AUTHOR` service actor | Founder Office plus repository maintainer |
...
| `CRM_MCP_ENABLED`, `CRM_MCP_ACTOR_ID` | Sensitive runtime configuration | Exact `true` enables the six-tool CRM endpoint (otherwise 503); UUID of the `AdminUser` with `affiliate.manage` recorded as the delegating actor for every CRM write and audit row | Founder Office plus repository maintainer |
| `LEARN_CONTENT_AUTONOMY_ENABLED` | Sensitive server-only kill switch | RFC-053 exact `true` permits new Learn editorial cycles only when the independent RFC-052 gate/configuration also passes; default/absence is off | Founder Office plus repository maintainer |
| `LEARN_CONTENT_LOCALES`, `LEARN_CONTENT_MIN_INTERVAL_HOURS`, `LEARN_CONTENT_OPENAI_MODEL` | Bounded server-only orchestration configuration | Published-language allowlist (initially `en`), whole-hour interval from 24 through 720 (default 24), and code-allowlisted high-quality managed-session model (initially only `gpt-6-astra`) | Repository maintainer under RFC-053 |
