...
Reconciled on 2026-08-11 for the approved B4GAMBLE canonical-domain release contract, GOOGLE-OAUTH-ACTIVATE-01 Preview canonical-host correction and MVP-RUNTIME-01 redacted runtime-binding inspection. Secret values are intentionally omitted.
...
| Production | **Blocked for a future deployment:** Prisma Postgres `prisma-postgres-cobalt-school` (`store_1I4F54ETrwSKS42o`) aliases are Production-only, but current base `DATABASE_URL`/`DIRECT_URL` values are empty | **Detected:** Production-only Better Auth/admin configuration | Production authority only when separately approved and configured | Governed Production data and mutations only | `main` Vercel Production deployment |
...
- **Current detected exception, refreshed 2026-08-11:** Preview and Production provider aliases remain populated but both environments' base runtime `DATABASE_URL` and `DIRECT_URL` are empty. Redacted provider-alias fingerprints were retrieved and are `DIFFERENT`; all detected aliases are direct rather than pooled. PROGRAM-AI and MVP-RUNTIME shared activation/migration verification remain blocked until matched pooled/direct bindings are restored without changing environment isolation.
